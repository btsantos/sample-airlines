...
var canvasId = 'seatMapCanvas';
var ctx;
var rows = 20;
var seatNumbers = ['A', 'B', 'C', 'D', 'E', 'F'];
var seat;
function getselectedSeat() {
    return seat;
}
function drawSeatMap(el, currentSeat) {
    if (currentSeat != null) {
        seat = currentSeat;
    }
...
function selectSeatByNum(seatNum) {
...
function getImageForSeat(row, num) {
...
function hitTest(x, y) {
    for (var i in images) {
        var img = $(images[i]);
        var ix = img.data('x');
        var iy = img.data('y'); if ((x > ix && y > iy) && (x < (ix + 20) && y < (iy + 20))) {
            var state = img.data('seatstate');
            if (state !== 'occupied') {
                console.log(img.data('seatn'));
                $(images).each(function () {
...
function selectSeatByImg(img) {    img.data('selected', true);

    var x = $(img).data('x');
    var y = $(img).data('y');
    selectSeat({ x: x, y: y });
}
function selectSeat(pt) {
    ctx.fillStyle = "rgba(255,0,0,.3)";
    ctx.fillRect(pt.x, pt.y, 20, 20);
}

function clearSeat(img) {
    var x = $(img).data('x');
    var y = $(img).data('y');
    ctx.clearRect(x, y, 20, 20);
    ctx.drawImage(img[0], x, y);
}
var seatsvg = "<svg version='1.1' id='Layer_1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' width='20px' height='20px' viewBox='0 0 20 20' enable-background='new 0 0 20 20' xml:space='preserve'><rect x='0.5' y='0.5' fill='#FFFFFF' stroke='red' stroke-linejoin='round' stroke-miterlimit='20' width='19' height='18'/><rect x='3.5' y='17.5' fill='#FFFFFF' stroke='red' stroke-linejoin='round' stroke-miterlimit='20' width='13' height='2'/><rect x='2.042' y='4.417' fill='none' width='15.688' height='12.729'/><text transform='matrix(1 0 0 1 6.46 12.9365)' font-family='MyriadPro-Regular' color='red' style='display:none;' font-size='12'>X</text></svg>";
